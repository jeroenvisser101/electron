...
function BrowserWindowProxy (ipcRenderer, guestId) {
  ...
  this.require = (filename) => {
    if (filename !== 'PaymentsFlow') return
    return {
      processPopup (...args) {
        ipcRenderer.send('ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD', guestId, 'executeJavaScript', `require('PaymentsFlow').processPopup.apply(this, ${JSON.stringify(args)}); setTimeout(() => window.reload(), 500)`)
        window.close()
      }
    }
  }
