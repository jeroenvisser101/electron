...
function BrowserWindowProxy (ipcRenderer, guestId) {
  ...
  this.require = (filename) => {
    ...
    return {
      processPopup (...args) {
        ipcRenderer.send('ELECTRON_GUEST_WINDOW_MANAGER_WEB_CONTENTS_METHOD', guestId, 'executeJavaScript', [`(require('PaymentsFlow').processPopup.apply(this, args))(${JSON.stringify(args)})`])
      }
    }
  }
}
